...
try {
    $sql = "SELECT DISTINCT
            ...
            WHERE (`ctb_doc`.estado =1 AND SUBSTRING(`ctb_libaux`.`cuenta`,1,$long_ini) BETWEEN $cuenta_ini AND $cuenta_fin);";
    $res = $cmd->query($sql);
        ...
        foreach ($cuentas as $cta) {
            try {
                $sql = "SELECT
                            ...
                            , IF(LENGTH(`ctb_libaux`.`id_tercero`) >=1,`ctb_libaux`.`id_tercero`,`ctb_doc`.`id_tercero`) AS id_tercero
                            , `ctb_doc`.`detalle`
                            , `ctb_libaux`.`debito`
                            , `ctb_libaux`.`credito`
                        FROM
            ...
            try {
                $sql = "SELECT
                        ...
                        FROM
                            ...
                            INNER JOIN `ctb_doc` 
                                ON (`ctb_libaux`.`id_ctb_doc` = `ctb_doc`.`id_ctb_doc`)
                        WHERE (`ctb_doc`.`fecha` <'$fecha_inicial' AND  `ctb_libaux`.`cuenta`='{$cta['cuenta']}' AND `ctb_doc`.`estado`=1 )
                        GROUP BY `ctb_libaux`.`cuenta`, `ctb_libaux`.`debito`, `ctb_libaux`.`credito`;";
                $res = $cmd->query($sql);
                $saldo_ini = $res->fetch();
            } catch (PDOException $e) {
                echo $e->getCode() == 2002 ? 'Sin Conexión a Mysql (Error: 2002)' : 'Error: ' . $e->getCode();
        ...
        ?>
            ...
            <table class="table-bordered bg-light" style="width:100% !important;" border=1>
                ...
                foreach ($movimientos as $tp) {
                    ...
                    $key = array_search($tp['id_tercero'], array_column($terceros, 'id_tercero'));
