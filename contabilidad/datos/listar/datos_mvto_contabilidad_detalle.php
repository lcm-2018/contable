...
if (!empty($listappto)) {
    ...
    foreach ($listappto as $lp) {
        ...
        if ((intval($permisos['editar'])) === 1) {
            ...
            $borrar = '<a value="' . $id_ctb . '" onclick="eliminarRegistroDetalle(' . $id . ')"class="btn btn-outline-danger btn-sm btn-circle shadow-gb" title="Borrar"><span class="fas fa-trash-alt fa-lg"></span></a>';
        } else {
            $editar = null;
            $detalles = null;
        }
