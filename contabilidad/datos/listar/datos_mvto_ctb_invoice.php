...
if (isset($_POST['rad']) && $_POST['rad']) {
    $andwhere .= " AND pto_rad.id_manu LIKE '%" . $_POST['rp'] . "%'";
}
    ...
    $sql = "SELECT
            ...
            FROM
                ...
                FROM
                    ...
                    LEFT JOIN `ctb_fuente` 
                        ON (`ctb_doc`.`id_tipo_doc` = `ctb_fuente`.`id_doc_fuente`)
                WHERE (`ctb_fuente`.`cod` = '$cod_ctb_doc' AND `ctb_doc`.`id_vigencia` = $id_vigencia)
                GROUP BY `pto_rad`.`id_pto_rad`, `ctb_doc`.`id_ctb_doc`) AS `t1`";
    $rs = $cmd->query($sql);
    $total = $rs->fetch();
...
if (!empty($listappto)) {
    foreach ($listappto as $lp) {
        ...
        $id_manu_rp = $lp['id_rad'] != '' ? $lp['id_rad'] : '-';

        $fecha = date('Y-m-d', strtotime($lp['fecha']));

        $base = base64_encode('0|' . $cod_ctb_doc . '|' .  $id_ctb);
        if (PermisosUsuario($permisos, 5501, 1) || $id_rol == 1) {
            $detalles = '<a text ="' . $base . '" onclick="cargarListaDetalleCtbInvoice2(this)" class="btn btn-outline-warning btn-sm btn-circle shadow-gb"  title="Detalles"><span class="fas fa-eye fa-lg"></span></a>';
        }
