(function ($) {
  ...
  $(document).ready(function () {
    ...
    $("#tableMvtoContableDetalle").DataTable({
      ...
      columns: [{ data: "cuenta" }, { data: "tercero" }, { data: "debito" }, { data: "credito" }, { data: "botones" }],
      order: [[0, "desc"]],
  ...
  $("#divModalForms").on("click", "#btnAddPucExcel", function () {
    if ($("#file").val() === "") {
      ...
      var btns =
        '<button class="btn btn-primary btn-sm" id="btnConfirCargaPuc">Aceptar</button><button type="button" class="btn btn-secondary  btn-sm"  data-dismiss="modal">Cancelar</button>';
      $("#divModalConfDel").modal("show");
...
function cambiaListadoContable(dato) {
  $('<form action="lista_documentos_mov.php" method="POST">' + '+<input type="hidden" name="id_doc" value="' + dato + '" />' + "</form>")
    .appendTo("body")
    .submit();
}
/*
...
const generaMovimientoCxp = (boton) => {
  InactivaBoton(boton);
  var val_fac = $("#valFactura")
    .text()
    .replace(/[\s$]+/g, "")
    .replace(/\,/g, "");
  var val_inp = $("#valImputacion")
    .text()
    .replace(/[\s$]+/g, "")
    .replace(/\,/g, "");
  var val_cos = $("#valCentroCosto")
    .text()
    .replace(/[\s$]+/g, "")
    .replace(/\,/g, "");
    ...
    success: function (r) {
      setTimeout(function () {
        ...
        $(
          '<form action="' +
            window.urlin +
            '/financiero/reporte_excel.php" method="post">' +
