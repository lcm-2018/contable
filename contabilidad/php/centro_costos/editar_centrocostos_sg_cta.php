...
try {
    ...
    if ((PermisosUsuario($permisos, 5508, 2) && $oper == 'add' && $_POST['id_cec_sgcta'] == -1) ||
        ...
        if ($id_cec_sg > 0) {
            if ($oper == 'add') {
                $id = $_POST['id_cec_sgcta'];
                $id_subgrupo = $_POST['id_subgrupo'];
                $id_cta = $_POST['id_txt_cta_con'] ? $_POST['id_txt_cta_con'] : 'NULL';

                if ($id == -1) {
                    $sql = "INSERT INTO tb_centrocostos_subgr_cta_detalle(id_cecsubgrp,id_subgrupo,id_cuenta)  
                            VALUES($id_cec_sg,$id_subgrupo,$id_cta)";
                    $rs = $cmd->query($sql);
                    ...
                    $sql = "UPDATE tb_centrocostos_subgr_cta_detalle 
                            SET id_cuenta=$id_cta
                            WHERE id_cecsubgrp_det=" . $id;
                    $rs = $cmd->query($sql);
            ...
            $res['mensaje'] = 'Primero debe guardar el Registro de Cuentas Contables';
        }
    } else {
