...
try {
    ...
    INNER JOIN `pto_documento` 
     ...
     WHERE `pto_documento_detalles`.`id_documento` ='$cdp' AND tipo_mov='CRP';";
    $res = $cmd->query($sql);
...
?>
<div class="px-0">
    <form id="modLiberaCrp">
        <div class="shadow mb-3">
            <div class="card-header" style="background-color: #16a085 !important;">
                <h6 style="color: white;"><i class="fas fa-lock fa-lg" style="color: #FCF3CF"></i>&nbsp;VALORES A LIQUIDAR DE CRP </h5>
                    <input type="hidden" id="id_cdp_doc" name="id_cdp_doc" value="<?php echo $cdp; ?>">
                    <input type="hidden" id="id_doc_neo" name="id_doc_neo" value="">
            </div>
            <div class="pt-3 px-3">
                <div class="row">
                    <div class="col-3">
                    ...
                    <div class="col-3">
                        <div class="col"> <input type="date" class="form-control form-control-sm" id="fecha" name="fecha" required value="<?php echo $fecha; ?>" min="<?php echo $fecha_cierre; ?>" max="<?php echo $fecha_max; ?>"></div>
                    </div>
                </div>
                <div class="row">
                    <div class="col-3">
                    ...
                    <div class="col-8">
                        <div class="col"> <textarea id="objeto" type="text" name="objeto" class="form-control form-control-sm py-0 sm" aria-label="Default select example" rows="3" required="required"></textarea></div>
                    </div>
                </div>
                <div class="row">
                    <div class="col-3">
                    ...
                    <div class="col-3">
                        <div class="col-2"><button type="button" class="btn btn-danger btn-sm " onclick="EnviarLiquidarCrp('')">Guardar</button></div>
                    </div>
                </div>
                        ...
                        <table id="tableListaCdp" class="table table-striped table-bordered  table-sm table-hover " style="width: 97%;">
                            ...
                            <tbody>
                                ...
                                foreach ($cdps as $lp) {
                                    ...
                                    try {
                                        $sql = "SELECT sum(valor) as registrado FROM pto_documento_detalles WHERE id_pto_doc = '$lp[id_pto_doc]' AND rubro ='$lp[rubro]' AND tipo_mov ='COP'";
                                        $res = $cmd->query($sql);
                                    ...
                                    try {
                                        $sql = "SELECT sum(valor) as liberado FROM pto_documento_detalles WHERE id_auto_crp = '$lp[id_pto_doc]' AND rubro ='$lp[rubro]' AND tipo_mov ='LRP'";
                                        $res = $cmd->query($sql);
                                    ...
                                    $campo = '<div class="btn-group">
                                                 <input type="text" class="form-control form-control-sm text-right" id="valor' . $j . '" name="valor' . $j . '" required value="' . $saldo2 . '" min="0" max="' . $saldo . '" onkeyup="valorMiles()">
                                    ...
                                    echo '<td class="text-right">' . $campo  . '</td>';
                                    echo '</tr>';
                                    $j++;
                                    $saldo = 0;
                                    $saldo2 = 0;
