...
$titulo = '';
 switch($id_reporte){
    case '3':
        $titulo = 'REPORTE DE LOTES A VENCERSE (<=' . $_POST['dias_ven'] .' DÍAS) - AGRUPADO POR SEDE-BODEGA-SUBGRUPO';
        break;
}
...
if (isset($_POST['dias_ven']) && strlen($_POST['dias_ven'])) {
    $where .= " AND DATEDIFF(far_medicamento_lote.fec_vencimiento,NOW()) BETWEEN 0 AND " . $_POST['dias_ven'];
}    
...
try {
    ...
    $sql = "SELECT far_medicamento_lote.id_lote,far_medicamentos.cod_medicamento,far_medicamentos.nom_medicamento,
                ...
                far_medicamento_lote.fec_vencimiento,DATEDIFF(far_medicamento_lote.fec_vencimiento,NOW()) AS dias
            FROM far_medicamento_lote
            INNER JOIN far_medicamentos ON (far_medicamentos.id_med=far_medicamento_lote.id_med)
            INNER JOIN far_bodegas ON (far_bodegas.id_bodega = far_medicamento_lote.id_bodega)
            INNER JOIN tb_sedes_bodega ON (tb_sedes_bodega.id_bodega = far_bodegas.id_bodega)
    ...
    <table style="width:100% !important">       
        <tbody style="font-size: 60%;">
            ...
            switch($id_reporte){
                case '3':
                    $tabla = '<tr style="background-color:#CED3D3; text-align:center">
                        <th>Código Art.</th><th>Nombre</th><th>ID Lote</th><th>Lote</th><th>Fecha Vencimiento</th><th>Vence en Días</th>
                        <th>Existencia</th><th>Vr. Promedio</th><th>Vr. Parcial</th><th>Vr. Total</th></tr>';
                    break;    
            }
            ...
            foreach ($objs as $obj1) {
                ...
                $tabla .= '<tr><th colspan="9" style="text-align:left">' . strtoupper($obj1['nom_sede'] . ' - ' . $obj1['nom_bodega']) . '</th><th style="text-align:right">' . formato_valor($obj1['val_total_sb']) . '</th></tr>';
                ...
                foreach ($objg as $obj2) {
                    ...
                    $tabla .= '<tr><th colspan="8" style="text-align:left">' . str_repeat('&nbsp',10) . strtoupper($obj2['nom_subgrupo']) . '</th><th style="text-align:right">' . formato_valor($obj2['val_total_sg']) . '</th></tr>';
