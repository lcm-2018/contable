...
$id_componente = isset($_POST['id']) ? $_POST['id'] : exit('Acción no permitida');
try {
    ...
    $sql = "SELECT
            ...
            WHERE `seg_entra_detalle_activos_fijos`.`id_acfijo` = '$id_componente'";
    $rs = $cmd->query($sql);
    $detalle_af = $rs->fetch();
    $cmd = null;
...
try {
    $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
    $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_WARNING);
    $sql = "SELECT `id_serial`, `id_activo_fijo`, `placa`, `num_serial` FROM `seg_num_serial` WHERE `id_activo_fijo`= '$id_componente'";
    $rs = $cmd->query($sql);
    $seriesc = $rs->fetchAll();
    $cmd = null;
} catch (PDOException $e) {
...
<div class="px-0">
    <div class="shadow">
        ...
        <div class="px-2">
            <form id="formUpComponenteActFijo">
                <input name="tipoEntrada" hidden value="<?php echo $tipo ?>">
                <input name="id_comp_actfijo" hidden value="<?php echo $id_componente ?>">
                <div class=" form-row">
                ...
                <div class="form-row">
                    <input type="hidden" id="cantidad" name="cantidad" value="<?php echo $detalle_af['cantidad'] ?>">
    ...
    <div class="text-center pt-3">
        <button id="btnUpComponenteAcFijo" type="button" class="btn btn-primary btn-sm">Actualizar</button>
