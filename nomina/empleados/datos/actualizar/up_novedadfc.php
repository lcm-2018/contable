...
<div class="px-0">
    <div class="shadow">
        ...
        <form id="formUpNovFc">
            <input type="number" name="numidnovafp" value="<?php echo $novafp['id_novfc'] ?>" hidden="true">
            <div class="form-row px-4 pt-2">
                <div class="form-group col-md-6">
                    <label for="slcUpNovAfp" class="small">Fondo cesantías</label>
                    <select id="slcUpNovAfp" name="slcUpNovAfp" class="form-control form-control-sm py-0" aria-label="Default select example">
                        ...
                        foreach ($afps as $a) {
                            if ($a['id_fc'] !== $idaf) {
                                echo '<option value="' . $a['id_fc'] . '">' . $a['nombre_fc'] . '</option>';
                            } else {
                                echo '<option selected value="' . $a['id_fc'] . '">' . $a['nombre_fc'] . '</option>';
                            }
                        }
                        ?>
                        <select>
            ...
            <div class="form-row px-4 pt-2">
                <div class="text-center mb-3">
                    <button class="btn btn-primary btn-sm actualizarFc">Actualizar</button>
