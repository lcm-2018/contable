...
try {
    ...
    $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_WARNING);
    $sql = "SELECT 
                `id_empleado`, `vigencia`, `salario_basico`, `no_documento`, CONCAT_WS(' ', `apellido1`, `apellido2`, `nombre1`, `nombre2`) AS `nombre`
            FROM
                (SELECT  
            ...
            WHERE `mes`  BETWEEN '$mesi' AND '$mesf' AND `tipo_liq` = 'R' GROUP BY `id_empleado`";
    $rs = $cmd->query($sql);
    $obj = $rs->fetchAll();
    $cmd = null;
} catch (PDOException $e) {
...
try {
    $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
    $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_WARNING);
    $sql = "SELECT id_empleado, mes_lic, anio_lic, SUM(dias_liqs) AS dias_liqs, SUM(val_liq) AS val_liq
            FROM
                nom_liq_licmp
            INNER JOIN nom_licenciasmp 
                ON (nom_liq_licmp.id_licmp = nom_licenciasmp.id_licmp)
            WHERE mes_lic BETWEEN '$mesi' AND '$mesf' AND anio_lic ='$anio' AND tipo_liq = 'R'
...
try {
    $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
    $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_WARNING);
    $sql = "SELECT id_empleado, mes_vac, anio_vac, SUM(dias_liqs) AS dias_liqs, SUM(val_liq) AS val_liq
            FROM
                nom_liq_vac
            INNER JOIN nom_vacaciones
                ON (nom_liq_vac.id_vac = nom_vacaciones.id_vac)
            WHERE mes_vac  BETWEEN '$mesi' AND '$mesf' AND anio_vac = '$anio' AND tipo_liq = 'R' GROUP BY id_empleado";
    $rs = $cmd->query($sql);
    $vac = $rs->fetchAll();
    $cmd = null;
} catch (PDOException $e) {
...
try {
    $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
    $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_WARNING);
    $sql = "SELECT id_empleado, mes_liq, anio_liq, SUM(dias_liq) AS dias_liq, SUM(val_liq_dias) AS val_liq_dias, SUM(val_liq_auxt) AS val_liq_auxt
            FROM
                nom_liq_dlab_auxt
            WHERE mes_liq BETWEEN '$mesi' AND '$mesf'  AND anio_liq = '$anio' AND tipo_liq = 'R' GROUP BY id_empleado";
    $rs = $cmd->query($sql);
    $dlab = $rs->fetchAll();
    $cmd = null;
} catch (PDOException $e) {
...
try {
    ...
    $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_WARNING);
    $sql = "SELECT 
                `id_empleado`, SUM(`val_vacacion`) AS `val_vacacion`, SUM(`val_cesantia`) AS `val_cesantia`, SUM(`val_interes_cesantia`) AS `val_interes_cesantia`, SUM(`val_prima`) AS `val_prima`, SUM(`val_prima_vac`) AS `val_prima_vac`, SUM(`val_prima_nav`) AS `val_prima_nav`, SUM(`val_bonifica_recrea`) AS `val_bonifica_recrea`
            FROM
                nom_liq_prestaciones_sociales
            WHERE mes_prestaciones  BETWEEN '$mesi' AND '$mesf' AND anio_prestaciones = '$anio' AND tipo_liq = 'R' GROUP BY id_empleado";
    $rs = $cmd->query($sql);
    $presoc = $rs->fetchAll();
    $cmd = null;
} catch (PDOException $e) {
...
try {
    ...
    $sql = "SELECT 
                `id_empleado`
                ,SUM(`aporte_salud_emp`) AS `aporte_salud_emp`
                ,SUM(`aporte_pension_emp`) AS `aporte_pension_emp`
                ,SUM(`aporte_solidaridad_pensional`) AS `aporte_solidaridad_pensional`
                ,`porcentaje_ps`
                ,SUM(`aporte_salud_empresa`) AS `aporte_salud_empresa`
                ,SUM(`aporte_pension_empresa`) AS `aporte_pension_empresa`
                ,SUM(`aporte_rieslab`) AS `aporte_rieslab`
            FROM
                nom_liq_segsocial_empdo
            WHERE mes BETWEEN '$mesi' AND '$mesf' AND anio = '$anio' AND tipo_liq = 'R' GROUP BY id_empleado";
    $rs = $cmd->query($sql);
    $segsoc = $rs->fetchAll();
    $cmd = null;
} catch (PDOException $e) {
...
try {
    $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
    $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_WARNING);
    $sql = "SELECT id_empleado, SUM(val_aporte) AS val_aporte
            FROM
            ...
            WHERE mes_aporte BETWEEN '$mesi' AND '$mesf' AND anio_aporte = '$anio' AND tipo_liq = 'R' GROUP BY id_empleado";
    $rs = $cmd->query($sql);
    $sind = $rs->fetchAll();
    $cmd = null;
} catch (PDOException $e) {
...
try {
    ...
    $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_WARNING);
    $sql = "SELECT 
                id_empleado, SUM(val_liq) AS tot_he, mes_he, anio_he
            FROM
                nom_liq_horex
            INNER JOIN nom_horas_ex_trab 
                ON (nom_liq_horex.id_he_lab = nom_horas_ex_trab.id_he_trab)
            WHERE mes_he BETWEEN '$mesi' AND '$mesf' AND anio_he = '$anio' AND tipo_liq = 'R'
            GROUP BY id_empleado";
    $rs = $cmd->query($sql);
    $hoex = $rs->fetchAll();
    $cmd = null;
...
try {
    $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
    $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_WARNING);
    $sql = "SELECT id_empleado, SUM(val_liq) AS val_liq, fec_reg
...
try {
    ...
    $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_WARNING);
    $sql = "SELECT 
                id_empleado, SUM(val_sena) AS val_sena, SUM(val_icbf) AS val_icbf, SUM(val_comfam) AS val_comfam
            FROM nom_liq_parafiscales
            WHERE mes_pfis  BETWEEN '$mesi' AND '$mesf' AND anio_pfis = '$anio' AND tipo_liq = 'R'
            GROUP BY id_empleado";
    $rs = $cmd->query($sql);
    $pfis = $rs->fetchAll();
    $cmd = null;
                        ...
                        <div class="card-header" id="divTituloPag">
                            <div class="row">
                                <div class="col-md-11">
                                    ...
                                    NÓMINA LIQUIDADA: RETROACTIVO MESES <?php echo $mesi . ' A ' . $mesf . ' DE ' . $anio ?>
