...
$ide = isset($_POST['numIdAfp']) ? $_POST['numIdAfp'] : exit('Acción no permitida');
$n = $_POST['txtNitUpAfp'];
...
try {
    ...
    $sql = "UPDATE nom_afp SET nombre_afp = ?, nit_afp = ?, dig_verf = ?, telefono = ?, correo = ? WHERE id_afp = ?";
    $sql = $cmd->prepare($sql);
    ...
    if ($updata > 0) {
        $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
        $sql = "UPDATE nom_afp SET fec_act = ? WHERE id_afp = ?";
        $sql = $cmd->prepare($sql);
