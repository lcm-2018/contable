...
$ide = isset($_POST['numIdEps']) ? $_POST['numIdEps'] : exit('Acción no permitida');
$n = $_POST['txtNitUpEps'];
...
try {
    ...
    $sql = "UPDATE nom_epss SET nombre_eps = ?, nit = ?, digito_verific = ?, telefono = ?, correo = ? WHERE id_eps = ?";
    $sql = $cmd->prepare($sql);
    ...
    if ($updata > 0) {
        $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
        $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_SILENT);
        $sql = "UPDATE nom_epss SET fec_act = ? WHERE id_eps = ?";
        $sql = $cmd->prepare($sql);
